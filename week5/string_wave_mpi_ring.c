...
int main(int argc, char **argv)
{
    ...
    for (int t = 0; t < time_steps; t++)
    {
        ...
        double left_recv = 0.0;
        double right_recv = 0.0;

        // 🔁 ALWAYS communicate (no if statements)
        MPI_Sendrecv(&local_positions[0], 1, MPI_DOUBLE, left, 0,
                     &right_recv, 1, MPI_DOUBLE, right, 0,
                     MPI_COMM_WORLD, MPI_STATUS_IGNORE);

        MPI_Sendrecv(&local_positions[local_n - 1], 1, MPI_DOUBLE, right, 1,
                     &left_recv, 1, MPI_DOUBLE, left, 1,
                     MPI_COMM_WORLD, MPI_STATUS_IGNORE);
